...
Flow LayerRegion::bridging_flow(FlowRole role, bool force_thick_bridges) const
{
    ...
    // Hacky. None of this weird thick-bridge stuff for bridge infills. Same spacing, but separate flow rates as a jic.
    ...
    if (print_object.config().thick_bridges) {
        ...
        return Flow::bridging_flow(float(sqrt(flow_ratio)) * nozzle_diameter, nozzle_diameter);
    } else {
        // The same way as other slicers: Use normal extrusions. Apply bridge_flow_ratio while maintaining the original spacing.
        return this->flow(role).with_flow_ratio(flow_ratio, space_ratio);
    }
}
